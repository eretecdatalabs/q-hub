 ...
 * @param {string} [params.session_id] - Optional session ID to group files by session.
 *
 ...
 */
async function uploadLocalFile({ req, file, file_id, session_id }) {
  const inputFilePath = file.path;
  const inputBuffer = await fs.promises.readFile(inputFilePath);
  const bytes = Buffer.byteLength(inputBuffer);

  const { uploads } = req.app.locals.paths;
  const userPath = session_id
    ? path.join(uploads, req.user.id, session_id)
  ...
  const filepath = session_id
    ? path.posix.join('/', 'uploads', req.user.id, session_id, path.basename(newPath))
